...
// ---------- JOGADOR aceita/recusa CONVITE recebido ----------
    ...
    // ---------- Forward para handlers separados ----------
    if (action === "chat") {
      const mod = await import("@/game/api-handlers/guild-chat");
      return mod.POST(req);
    }
    if (action === "shop" || action === "buy_upgrade" || action === "buy_bonus") {
      const mod = await import("@/game/api-handlers/guild-shop");
      return mod.POST(req);
    }
    if (action === "skills" || action === "invest_skill" || action === "reset_skills") {
      const mod = await import("@/game/api-handlers/guild-skills");
      return mod.POST(req);
    }
    if (action === "guild_boss" || action === "boss_attack" || action === "boss_enter" || action === "boss_leave") {
      const mod = await import("@/game/api-handlers/guild-boss");
      return mod.POST(req);
    }
    if (action === "guild_war" || action === "war_attack" || action === "war_join" || action === "war_leave" || action === "war_history") {
      const mod = await import("@/game/api-handlers/guild-war");
      return mod.POST(req);
    }
